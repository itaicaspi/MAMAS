...
class LC3
{
	...
	void updateFlags(short val); // get NZP flags
	bool fixDataHazard(unsigned short reg1, unsigned short reg2, struct Signals &signals);
public:
	LC3();
	void DumpMem();
	...
	void Fetch(struct Signals &signals);
	void Decode(struct Signals &signals);
	void Exec(struct Signals &signals);
	void WbMem(struct Signals &signals);
